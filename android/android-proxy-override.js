...
const proxyOverride = () => {
  Java.perform(() => {
    ...
    Java.use("java.lang.System").setProperty(
      "http.proxyPort",
      PROXY_PORT.toString()
    );
    ...
    Java.use("java.lang.System").clearProperty.implementation = function (
      property
    ) {
      if (controlledSystemProperties.includes(property)) {
        if (DEBUG_MODE)
          console.log(`Ignoring attempt to clear ${property} system property`);
        return this.getProperty(property);
      }
      return this.clearProperty(...arguments);
    };
    Java.use("java.lang.System").setProperty.implementation = function (
      property
    ) {
      if (controlledSystemProperties.includes(property)) {
        if (DEBUG_MODE)
          console.log(
            `Ignoring attempt to override ${property} system property`
          );
        return this.getProperty(property);
      }
    ...
    ConnectivityManager.getDefaultProxy.implementation = () =>
      ProxyInfo.$new(PROXY_HOST, PROXY_PORT, "");
    // (Not clear if this works 100% - implying there are ConnectivityManager subclasses handling this)

    console.log(
      `== Proxy system configuration overridden to ${PROXY_HOST}:${PROXY_PORT} ==`
    );
    ...
    const proxySelectorClasses = Java.enumerateMethods(
      "*!select(java.net.URI): java.util.List/s"
    ).flatMap((matchingLoader) =>
      matchingLoader.classes
    ...
    console.log(
      `== Proxy configuration overridden to ${PROXY_HOST}:${PROXY_PORT} ==`
    );
  });
};
setTimeout(proxyOverride, timeout);
