    ...
    try {
      ...
      const matchOkHttpChain = (cls, expectedReturnTypeName) => {
        ...
        const matchingFields = fields.filter(
          ([_, field]) => field.fieldReturnType?.className === argumentTypeName
        );
      ...
      const buildUnhandledErrorPatcher = (
        errorClassName,
        originalConstructor
      ) => {
        return function (errorArg) {
          try {
            ...
            const exceptionStackIndex = stackTrace.findIndex(
              (stack) => stack.getClassName() === errorClassName
            );
            ...
            callingMethod.overloads.forEach((failingMethod) => {
              ...
              if (isOkHttpCheckMethod(errorMessage, failingMethod)) {
                ...
                console.log(
                  `      [+] ${className}->${methodName} (fallback OkHttp patch)`
                );
              } else if (
                isAppmattusOkHttpInterceptMethod(errorMessage, failingMethod)
              ) {
                // See Appmattus CertificateTransparencyInterceptor patch in unpinning script:
                const chainType = Java.use(
                  failingMethod.argumentTypes[0].className
                );
                const responseTypeName = failingMethod.returnType.className;
                const okHttpChain = matchOkHttpChain(
                  chainType,
                  responseTypeName
                );
                failingMethod.implementation = (chain) => {
                  if (DEBUG_MODE)
                    console.log(` => Fallback Appmattus+OkHttp patch`);
                  const proceed =
                    chain[okHttpChain.proceedMethodName].bind(chain);
                  const request = chain[okHttpChain.requestFieldName].value;
                  return proceed(request);
                };
                console.log(
                  `      [+] ${className}->${methodName} (fallback Appmattus+OkHttp patch)`
                );
              } else if (isX509TrustManager(callingClass, methodName)) {
                const argumentTypes = failingMethod.argumentTypes.map(
                  (t) => t.className
                );
                ...
                if (
                  ...
                  failingMethod.implementation = (certs, authType) => {
                    if (DEBUG_MODE)
                      console.log(
                        ` => Fallback X509TrustManager patch of ${className} base method`
                      );
                  ...
                  console.log(
                    `      [+] ${className}->${methodName} (fallback X509TrustManager base patch)`
                  );
                } else if (
                  argumentTypes.length === 3 &&
                  argumentTypes.every(
                    (t, i) => t === EXTENDED_METHOD_ARGUMENTS[i]
                  ) &&
                  ...
                  failingMethod.implementation = function (
                    ...
                    if (DEBUG_MODE)
                      console.log(
                        ` => Fallback X509TrustManager patch of ${className} extended method`
                      );
                  ...
                  console.log(
                    `      [+] ${className}->${methodName} (fallback X509TrustManager ext patch)`
                  );
                } else {
                  console.warn(
                    `      [ ] Skipping unrecognized checkServerTrusted signature in class ${callingClass.class.getName()}`
                  );
                }
              } else {
                console.error(
...
setTimeout(unPinningFallback, timeout);
