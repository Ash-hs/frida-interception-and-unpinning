  ...
  "com.android.okhttp.Address": [
    {
      ...
      replacement: (targetMethod) => {
        const defaultHostnameVerifier = Java.use(
          "com.android.okhttp.internal.tls.OkHostnameVerifier"
        ).INSTANCE.value;
        const defaultCertPinner = Java.use(
          "com.android.okhttp.CertificatePinner"
        ).DEFAULT.value;
  ...
  "com.worklight.wlclient.api.WLClient": [
    {
      ...
      getMethod: (WLClientCls) =>
        WLClientCls.getInstance().pinTrustedCertificatePublicKey,
      overload: "*",
    },
  ],
    ...
    classesToPatch.forEach((targetClassName) => {
      ...
      patches.forEach(({ methodName, getMethod, overload, replacement }) => {
        ...
        if (targetMethodImplementations.length === 0) {
          if (DEBUG_MODE)
            console.log(`[ ] ${targetClassName} ${methodDescription}`);
          return;
        }
      ...
      if (!patchApplied) {
        console.warn(
          `[!] Matched class ${targetClassName} but could not patch any methods`
        );
      }
    });
