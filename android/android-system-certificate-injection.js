  ...
  Java.perform(() => {
    ...
    try {
      ...
      cert = certFactory.generateCertificate(
        ByteArrayInputStream.$new(certBytes)
      );
    } catch (e) {
    ...
    [
      ...
      try {
        ...
        if (i === 0) {
          throw new Error(
            `${TrustedCertificateIndexClassname} not found - could not inject system certificate`
          );
        } else {
          // Other classnames are optional fallbacks
          if (DEBUG_MODE) {
            console.log(
              `[ ] Skipped cert injection for ${TrustedCertificateIndexClassname} (not present)`
            );
          }
          return;
        }
      ...
      if (DEBUG_MODE)
        console.log(
          `[+] Injected cert into ${TrustedCertificateIndexClassname}`
        );
    });
...
setTimeout(certificateInject, timeout);
